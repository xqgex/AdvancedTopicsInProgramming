...
#include "RSPPlayer_mycode.h"

REGISTER_ALGORITHM(mycode)

////////////////////////////////////////////////////////////////////
////	RSPPlayer_mycode 					////
////////////////////////////////////////////////////////////////////
RSPPlayer_mycode::RSPPlayer_mycode() {
	int i,j; // 1<=i<=M 1<=j<=N
...
////////////////////////////////////////////////////////////////////
void RSPPlayer_mycode::getInitialPositions(int player, std::vector<std::unique_ptr<PiecePosition>>& vectorToFill) {
...
void RSPPlayer_mycode::notifyOnInitialBoard(const Board& b, const std::vector<std::unique_ptr<FightInfo>>& fights) {
	int i,j; // 1<=i<=M 1<=j<=N
	int player_at;
...
void RSPPlayer_mycode::notifyOnOpponentMove(const Move& move) { // Called only on opponent's move
...
void RSPPlayer_mycode::notifyFightResult(const FightInfo& fightInfo) { // Called only if there was a fight
	int x,y,winner;
...
unique_ptr<Move> RSPPlayer_mycode::getMove() {
...
////////////////////////////////////////////////////////////////////
////	RSPPlayer_mycode functions 			////
////////////////////////////////////////////////////////////////////
// Return 0 if game is over for us and 1 otherwise
int RSPPlayer_mycode::checkGameOver() {
	int i,j; // 1<=i<=M 1<=j<=N
	for (i=1;i<=M_COLS;++i) { // M_COLS is x
		for (j=1;j<=N_ROWS;++j) { // N_ROWS is y
...
int RSPPlayer_mycode::fightSimulate(char piece_1, char piece_2) {
	int player_1_lose = 1;
	int player_2_lose = 1;
	switch (piece_1) {
...
// Guess what is the opponent's flag
void RSPPlayer_mycode::findTarget() {
	std::random_device rd;
	std::mt19937 mt(rd());
...
PointRPS RSPPlayer_mycode::findVictim(PointRPS my_piece) {
	int i,j; // 1<=i<=M 1<=j<=N
	for (i=1;i<=M_COLS;++i) { // M_COLS is x
		for (j=1;j<=N_ROWS;++j) { // N_ROWS is y
...
PointRPS RSPPlayer_mycode::getAvailablePoint(char piece) {
...
double RSPPlayer_mycode::scoring(PointRPS source_piece, PointRPS next_step) {
